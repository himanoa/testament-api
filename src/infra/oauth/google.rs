use oauth2::prelude::*;
use url::Url;
use infra::oauth::{OAuthProvider, VerifyTokenError, Claims};
...
impl OAuthProvider for GoogleProvider {
    ...
    fn verify_token(&self, token: &str, well_knwon_url: &str) -> Result<Claims, VerifyTokenError> {
        ...
        if segments.len() <= 2 {
            return Err(VerifyTokenError::SegmentSplitError { msg: "splited segments length <= 2".to_string() });
        }
        let envelope: Envelope = match base64::decode_config(segments[0], base64::STANDARD) {
            ...
            Err(_) => {
                return Err(VerifyTokenError::EnvelopeDecodeError { msg: "".to_string() });
            }
        };
